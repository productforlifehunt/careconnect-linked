...
export function AIMedicationHelper({ medications, caredOneName }: AIMedicationHelperProps) {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setAnalysis("Unable to analyze medications right now.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <CardContent>
        {!analysis && !loading ? (
          <Button variant="outline" size="sm" onClick={analyze} className="w-full text-xs">
            <Pill className="h-3 w-3 mr-1" /> Check {medications.length} Medication{medications.length > 1 ? "s" : ""} for Interactions
          </Button>
        ) : loading ? (
          ...
          <div className="text-xs text-foreground leading-relaxed whitespace-pre-wrap bg-muted/50 p-3 rounded-lg">
            ...
            <p className="mt-2 text-[10px] text-muted-foreground italic">
              ⚠️ AI analysis — always confirm with a pharmacist or doctor.
            </p>
          </div>
        )}
      </CardContent>
