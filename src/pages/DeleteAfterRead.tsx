...
const papers: Paper[] = [
  ...
  },
  {
    ...
    procedure: ["【P 原型·含人的角色设计】先培训 3 名同伴支持者（peer supporter，本身也是照护者）使用平台与干预材料。干预的一半是软件，另一半是被训练过的人——这一步等于在设计一个岗位，不只是一个界面。", "【T 测试·真实情境】9 名家庭照护者接受为期 2 周的技术支持干预，由这 3 名同伴支持者在真实生活情境中提供，而非实验室会话。", "【T 测试·双向取材】同时收集两侧反馈：照护者一侧（平台操作、内容是否对得上自己的处境）和同伴支持者一侧（支持关系好不好建立、平台是否妨碍沟通）。双向取材是本篇少见的优点。", "【T 测试·收口】把可用性、可接受性与初步效果三类资料合并解读；样本 9 人、2 周，只能说方向，不能说效果。", "【不属于五步法】同理、定义、构思缺席：平台与材料在研究开始前已定稿，本篇的定位是小规模先导（pilot）。"],
...
const projectDossiers: Record<string, ProjectDossier> = {
  "iSupport（WHO）+ 各国本土化版": {
    ...
    procedure: ["【P 原型·第 1 步就是搬运】拿 WHO 官方 iSupport 课程做翻译与回译（back-translation：译过去再由另一人译回来核对语义）。整个项目族起点不是需求调研，而是一份已经写好的世界卫生组织教材。", "【E 同理 + D 定义·合并进行】做利益相关者咨询 + 照护者焦点小组，目的只有一个：找出哪些说法在本地文化里说不通、哪些服务信息（热线、机构、补助）必须换成本地的。这一格是「校正」，不是「发现」。", "【P 原型】把改好的内容做成本地语言数字版（网页或桌面版），到此产品定稿。", "【T 测试】可用性场次固定四件套且顺序固定：先填 eHEALS（电子健康素养）→ 做 11 项结构化任务并出声思维 → 填 SUS → 半结构化访谈。族谱里多个国家版本用的是同一套四件套，可以直接照抄。", "【T 测试·实施层】后续研究把已适配版本放进基层医疗真实流程，观察效果与实施条件（谁来推、医生愿不愿意开、照护者会不会自己回来用）。", "【不属于五步法·构思格全族缺席】iSupport 全族没有任何一篇做过构思：从来没有人问过「除了做在线课程，还能做什么」。这是 WHO 品牌项目的结构性特征——形态先定，只允许本地化。"],
  ...
  "Tele-Savvy": {
    ...
    procedure: ["【P 原型·移植既有课程】把已有线下 Savvy Caregiver 课程原样重组为线上课程（Tele-Savvy）：改的是交付通道（视频会议 + 自学视频），不是教学内容本身。这是全族最省钱的做法——内容资产已经存在。", "【T 测试·真实系统内】在美国退伍军人医疗系统（VA）里作为示范项目实际交付，然后访谈照护者。注意这一步选在一个已经有病人流、有经费流、有人力的系统里做，而不是自己从零招募。", "【P 原型·迭代】按访谈反馈同时改两样：课程内容与实施流程（谁带、几周、每次多久、怎么把人留住）。流程改动往往比内容改动更决定成败。", "【T 测试·升级为验证】另外发表随机对照试验（RCT）协议，把招募、分组、干预与测量时间点预先写死。这一步的意义是把「大家觉得好用」升级成「可以写进指南的证据」。", "【不属于五步法】同理、定义、构思三格全部缺席：需求早在线下课程年代就定完了。Tele-Savvy 的可参考之处恰恰在于它跳过了这三格却拿到了最硬的证据。"],
  ...
  "CareVirtue": {
    ...
    procedure: ["【P 原型·产品先行】CareVirtue 作为已完成的网络平台先存在。第一个真实动作是主要照护者注册后把支持者（亲属、朋友、邻居）拉进来，形成一个照护网络——这一步的成败决定后面三篇论文有没有数据。", "【T 测试·长期真实使用】网络成员在日常照护中使用三类功能：共享日志、任务分派、成员间沟通。不是实验室任务，而是几周到数月的真实记录。", "【T 测试·第 1 篇（可行性）】收集问卷 + 访谈，回答「这东西家庭能不能真的用起来」，不回答疗效。", "【E 同理·第 2 篇（沟通）】把同一批人的资料重新切一次，专门分析网络成员之间的交流体验：谁愿意写、谁只看不写、共享日志会不会变成家庭内部的互相监督。", "【D 定义·第 3 篇（话题建模）】对用户生成内容（照护者自己写下的日志文本）与参与日志做计算分析（topic modeling），从真实文字里反推照护者到底在操心什么。这是全族谱里最值得抄的一招：不问人，直接读他们已经写下的东西。", "【一鱼三吃·必须点明】三篇论文共用同一批参与者与同一份数据，只是切法不同（可行性 / 沟通 / 计算）。这不是三个研究，是一次数据采集的三种发表形态。"],
  ...
  "Partner in Balance（马斯特里赫特）": {
    ...
    procedure: ["【E 同理 + D 定义】先做需求评估，确定课程要处理哪些主题与目标（多与照护者的心理适应、角色转变、自我效能相关）。这是族谱里少数真的先问人再动手的项目。", "【I 构思 + P 原型·双人组合】照护者与心理师一起迭代课程内容：照护者提供「真实处境」，心理师提供「干预机制」。两方合写是 Partner in Balance 最核心的做法——不让用户自由许愿，也不让专家闭门造课。", "【P 原型】做成线上自学模块，并配一名辅导者（coach）提供混合式支持（blended：线上内容 + 真人跟进）。同样是「软件 + 人」的组合干预。", "【T 测试·硬证据】随机对照试验比较干预组与对照组的前后变化，产出族谱里最硬的一批效果数据。", "【T 测试·实施层】干预结束后访谈：阻力是什么、什么条件下机构才推得动、人们会不会继续用。这一格回答的是「能不能落地」，与「有不有效」是两件事。", "【P 原型·再定制】针对早发性失智（young-onset）家庭把内容再改一版，因为这群人的处境（还在工作、有未成年子女、房贷）与老年发病完全不同。这是把一套内容复用到第二人群的标准动作。"],
  ...
  "Inlife / myinlife（同一课题组）": {
    ...
    procedure: ["【E 同理 + I 构思】与目标群体协商要做哪些功能与内容（in dialogue with target group）：这是 Inlife 项目公开声明的方法立场，也是它区别于 iSupport 的地方——形态不是预先给定的。", "【P 原型】部署 myinlife 在线社会支持平台，核心是帮照护者把身边可动员的人组织起来（社会支持网络），而不是给他上课。", "【T 测试·前后测】试点前后各测一次用户状态与平台使用数据，构成前后对照。", "【E 同理·事后】试点后访谈参与者，解释使用与不使用的原因。", "【不属于五步法·发表策略】把「过程评价」（怎么用起来的、哪里卡住）与「前后测结果」（状态有没有变）拆成两篇分别发表。这是纯粹的产出量策略，与设计质量无关，但值得照抄。"],
  ...
  "CIRCA / CIRCA-BC ✅已核实": {
    ...
    procedure: ["【E 同理·地点是变量】举行 6 场焦点小组，地点刻意分层：3 场城市、2 场农村、1 场独立生活机构；每场约 2 小时，每组最多 3 轮。城乡分层是因为「本地记忆」在城乡完全不同——这一步是整个项目的方法核心。", "【D 定义 + I 构思·素材即内容】筛选并整理出 475 张图片、58 段视频、105 首音乐，归入 7 个主题。注意：这里的「设计」不是画界面，而是选素材；内容的地方性就是产品价值本身。", "【P 原型】把这批本地素材装入现成的触屏 CIRCA 框架，得到不列颠哥伦比亚版 CIRCA-BC。软件外壳是复用的，只换内容。", "【T 测试·测的是关系不是界面】3 组「失智症患者 — 护理助手」配对各使用约 30 分钟。评价单位是一对人，不是一个用户。", "【T 测试·分析动作】全程录像录音，分析三件事：话轮（谁说、说多久、轮换是否平衡）、哪些素材真的触发了回忆和讲述、双方互动质量的变化。这是全表里最独特的评价方式——把「对话」当结局指标。", "【不属于五步法】没有量表、没有健康结局、样本 3 对；它的贡献是方法示范，绝不能引成疗效证据。"],
  ...
  "CAREGIVERSPRO-MMD（EU H2020 690211）": {
    ...
    procedure: ["【T 测试·三类人同测】基线让三类参与者同时接触并评价 CAREGIVERSPRO-MMD 平台：失智症患者、家庭照护者、以及相关专业人员。同一个平台被三种角色打分，是这个项目的结构特点。", "【T 测试·真实使用 1 周】使用三类功能整整 1 周：信息（疾病与照护知识）、社交（同类照护者互助）、自我管理（记录与自我追踪）。1 周是个诚实的时长——足够暴露「打开一次就不再回来」，又短到不需要长期经费。", "【T 测试·后测】一周后再评一次三项指标：满意度、感知有用性（perceived usefulness）、易用性（ease of use）。这三项来自技术接受模型，是同族里最常被复用的量表组合。", "【E 同理·靶向追问】只对评分低的项目追加开放式问题。这一招极省成本：不做全员访谈，只审问差评。强烈建议照抄。", "【T 测试·续篇】后续 J Appl Gerontol 论文用同一批数据继续分析参与度、可用性与有用性——又是一次采集、两次发表。"],
  ...
  "GamePlan4Care（REACH II 的网络版）": {
    ...
    procedure: ["【P 原型·搬循证干预上网】把已获证据支持的 REACH II 干预按模块整体搬到网页：安全、压力、健康、情绪、服务、支持、行为问题共七类。内容一行没重写，只换载体。", "【P 原型·加数字件】在搬运基础上加入网页才做得到的东西：风险评估、教育材料、可填写的表单练习、视频，以及 Dementia Care Navigator（照护导航）。", "【T 测试】31 名照护者在系统内完成指定任务并回答开放式问题；按测试目的分成技术组 18 人与内容组 13 人。", "【T 测试·留证】全程录音 + 录屏，保证每个卡点可回放、可计数，不依赖研究者记忆。", "【D 定义·反哺开发】两名研究者归纳编码，把结果切成两类修改单——导航问题、内容问题——交回开发团队。目标明确：在花大钱做随机试验之前，先别让界面把试验搞砸。", "【不属于五步法】同理、构思缺席；也不测任何临床结局。这是「RCT 前的排雷」，定位清楚，别引成疗效。"],
  ...
  "WECARE（华人照护者微信干预）✅已核实：有后续篇": {
    ...
    procedure: ["【D 定义·专家定骨架】4 名专家先把 7 周课程结构定下来（第几周讲什么）。骨架由专家拍板，用户不参与结构决策——这一点必须照抄，否则会陷入无止境的许愿。", "【E 同理 + P 原型·小规模文化改编】3 名照护者 + 3 名服务人员 + 2 名社区领袖共同做文化改编：把内容改成华人家庭真的听得进去的说法。总共只用了 8 个人，这就是「最小投入换足够合法性」的范本。", "【T 测试·极小样本】5 名目标用户以普通话出声思维测试两件事：前台导航与后台功能。5 个人足以抓出致命问题，再多是浪费。", "【P 原型·交付形态】向 24 名参与者推送 40 篇多媒体文章，每篇约 3–6 分钟。篇幅是刻意设计的——照护者只有碎片时间，长内容等于没内容。", "【T 测试·加真人环节】在第 3、5、7 周各举行一次线上小组会议，同时全程开放群聊与私聊。软件负责内容，真人负责留存。", "【T 测试·双轨测量】基线与干预后各测一次量表，同时用后台日志记录阅读量与活动参与。自报 + 日志双轨，是拿来对冲「说得好听但没打开」的标准手法。"],
  ...
  "Care Me Too（华人移民照护者 App）": {
    ...
    procedure: ["【E 同理·把结构性障碍问出来】先做访谈，问的不只是「App 想要什么功能」，而是心理压力、社会孤立、移民身份带来的障碍（语言、证件、对机构的不信任）与照护障碍。族谱里少见的把结构性劣势当需求来源的项目。", "【I 构思 + P 原型】在概念设计与原型阶段和照护者一起讨论内容与功能，两格连着走，但没有记录发散了几个方案。", "【T 测试·实验室段】22 人在实验室操作 App 并接受深度访谈——先在可控环境抓明显问题。", "【T 测试·居家段】同一批人把 App 带回家用 1 周，进入真实生活情境。实验室 + 居家两段式是本项目最值得抄的结构：便宜的一段先筛，贵的一段才验。", "【T 测试·随访】电话随访收集具体的设计与功能修改意见，形成下一版修改单。", "【不属于五步法】没有对照组、没有临床结局：产出是需求与修改方向，不是效果。"],
  ...
  "失能老人照护者 eHealth 求助工具（魁北克）": {
    ...
    procedure: ["【E 同理·治理先行】2017–2018 年在魁北克 11 个地区各设一个咨询委员会。先建治理结构再谈设计，是拿公共经费的项目的标准开局——它同时解决了招募渠道和政治合法性两件事。", "【E→P 五格全在这一步·共 8 次会议】举行 8 次需求共同设计会议，明确用了 7 类工具：卡片分类（信息归类）、人物画像（persona）、工具分析（现有工具比较）、头脑风暴（唯一真正的构思动作）、草图、原型、预测试。这是全表里工具链最完整的一条，可以整套照抄写进方法章。", "【D 定义】把上述产出收敛成信息架构（内容怎么组织）与功能需求（要做什么）两份可交付文件。", "【不属于五步法·会议本身成了论文】另一篇论文用社会正义框架重读整个参与过程，并把会议全部计数公开：24 次筹备会 + 11 次正式共同设计会 + 11 次总结会。请注意这个比例——筹备会比设计会还多一倍。这就是共同设计的真实成本结构：绝大部分工时花在开会前的开会上。"],
  ...
  "mWITH ME ✅已核实全文": {
    ...
    procedure: ["【D 定义·理论先行】先用两套现成理论搭需求与内容框架：NEBM（叙事循证医学）与马斯洛需求层次。先有框架再去访谈，访谈就变成往格子里填料——这是最省时间的做法，也是最容易被批评「理论决定结论」的做法。", "【E 同理】2024 年 9–12 月在医院老年科与神经科招募 18 人，每人访谈 20–40 分钟。招募地点选在门诊，等于让医院替你完成筛选。", "【D 定义】按 COREQ（质性研究报告规范）逐项报告，并把访谈主题逐条转成 App 设计条目。写清 COREQ 是让审稿人挑不出程序毛病的最低成本手段。", "【T 测试·换人】另招 20 名参与者（不是原来那 18 人）实际使用 3 周。", "【T 测试·中途保活】第 1 周做电话回访。这一步表面是收反馈，真实作用是防脱落——3 周使用期最怕第 4 天就没人打开。", "【T 测试·后测】3 周后完成四部分问卷加开放式反馈，形成可用性与可接受性结论。", "【不属于五步法】构思格缺席：功能来自理论框架映射，没有备选形态比较。"],
  ...
  "日本：名古屋 Web App（英文发表）": {
    ...
    procedure: ["【T 测试·基线】由 2 名研究护士面对面访问约 15 分钟完成基线测量。用护士而不是研究员执行，是为了在医疗场景里取得信任并压低拒答率。", "【P 原型·两件东西】照护者在自己手机上使用两部分：结构化知识模块（学东西）+ 匿名交流空间（说心里话）。匿名是刻意的——照护者最难说的话恰恰不能让家人看见。", "【T 测试·四时点】基线、第 1、2、3 个月各测一次，共 4 个时间点。三个月是「够长到能看变化、短到经费撑得住」的常见折中。", "【不属于五步法·必须留意的数字】另有 43 名感兴趣者自己进了虚拟社区但不计入正式样本。这个数字比正式样本更说明问题：自愿涌入的人远多于被正式纳入的人，而论文的结论只建立在后者身上。", "【不属于五步法】同理、定义、构思三格缺席：内容与形态在研究开始前已定稿。"],
  ...
  "DEM-DISC（荷兰）": {
    ...
    procedure: ["【P 原型·在既有工具上改】不重做系统：以已有的 DEM-DISC 评估结果为依据改进这套电子建议工具（根据个人情况给出该找哪种服务的建议）。", "【T 测试·整群随机】按照护网络/机构做整群随机分配（cluster randomisation），不是按个人随机。原因很实际——同一个机构内没法让一半人用一半人不用，会串味。", "【T 测试】干预组使用定制化建议；比较组按试验方案接受规定的对照条件。", "【T 测试·结局】在预设时间点比较照护相关与服务使用相关结局（例如是否真的用上了该用的服务）。注意它测的是服务利用，而不是照护者情绪——这类结局更容易做出显著结果，也更容易说服付钱的一方。", "【不属于五步法】同理、定义、构思、原型迭代全部不在本项目范围：这是评价型研究，工具早已存在。"],
  ...
  "Hispanic 家庭照护者·失智功能分期 Web App（美国）★最高价值": {
    ...
    procedure: ["【E 同理·11 场工作坊】用三类材料开展 11 场参与式工作坊：问卷（结构化）、拼贴（collage，让说不出口的东西被贴出来）、录音录像（留证）。拼贴这一招对讲不清需求的人群特别有效，成本也低。", "【D 定义】4 名研究者依据多个概念模型对需求编码。四人编码是为了让「需求」看起来不是某一个人的主观判断。", "【I 构思 + P 原型·靠设计原则代替发散】把需求映射到 PSD（说服式系统设计原则）与自我管理原则，再据此做界面。这里没有真正的头脑风暴——发散被一套现成原则替代了，这是学术项目里最常见的偷渡方式。", "【T 测试】后续做启发式评价（专家按规则挑错）与可用性评价（真人操作）。", "【诚实标注】当前列出的 PMID 37379048 未能可靠匹配到该论文，因此后续评价阶段的样本量与结果无法确认，此处不补写任何数字。引用前必须自行核对。"],
  ...
  "CareFit（英国 Strathclyde）★两篇成套": {
    ...
    procedure: ["【E 同理 + I 构思·全线上、总工时封顶】首篇在 2020 年 7–8 月全程线上完成 3 次共同设计，工具只有三样：Zoom（开会）、MURAL（在线白板贴便签）、Qualtrics（会后问卷）。关键设计：每位参与者的总承诺时间明确封顶在 7 小时以内。这是全表最值得照抄的一条——先把参与成本写死，招募难度立刻下降。", "【I 构思 + D 定义·三次会各有分工】第 1 次：先展示一个简单原型和运动指南，再用 MoSCoW（Must/Should/Could/Won't 四档）与 keep/lose/change（留/删/改）给需求排序；第 2 次：复核第 1 次结论，并具体设计教育、运动、沟通三类内容；第 3 次：定稿。注意每次会议都先给东西再问意见，从不空手问「你想要什么」。", "【P 原型·会议与开发交错】每次会议后立刻做一轮 2–3 周的 Agile Scrum 冲刺，共产出 3 个版本；用 FURPS+ 把功能性与非功能性需求分开整理，并在模拟器加多种真实 Android 设备上测兼容。做法要点是：参与者看到的每一版都是可点的真东西，不是纸上讨论。", "【P 原型·换技术栈扩人群】后续把 CareFit 改写为 React Native 跨平台版本，通过社区网络与 Join Dementia Research（英国全国失智研究招募平台）招募，并给每名参与者一个专属应用商店链接——这既方便安装，也让每个下载都能对上人。", "【T 测试·8 周三轨采集】41 名失智症照护者自助使用 8 周（无教练陪伴），研究者并行采三种数据：线上问卷（SUS、EQ-5D-5L、IPAQ-SF 等）、页面级时间戳日志（Activity / Planner / Resources / Sharing 分开计时，能看出到底哪一页真被用）、以及约 30–45 分钟的访谈或焦点小组。", "【T 测试·诚实结论】41 人中仅 21 人完成（51%），且高基线活动组完成率 36%、低活动组 59%；作者据此明确表态：现在还不能进入随机对照试验。这个「主动认怂」的收尾比任何漂亮结论都更值得抄。"],
  ...
  "低保真原型 · 传感器交互平台（JMIR Form Res 2024）★最省钱": {
    ...
    procedure: ["【D 定义 + P 原型·不写一行代码】研究团队直接复用前序研究成果（464 人问卷 + 10 人访谈）作为需求依据，然后只用 Figma 做出可点击的低保真原型，覆盖三个场景：跌倒、激越、日常活动。全程零开发、零传感器、零后端——这是全表最省钱的一条，强烈建议照抄。", "【E 同理·先统一认知再问意见】每场开头先播一段 3.5 分钟的荷兰语说明视频，解释 Wi-Fi 感知、AI 计算、信息如何传给家属；看完才问正面/负面预期和实施前提。这一步解决了讲不清技术就问不出真意见的死结。", "【T 测试·5 项任务】参与者边出声思考边完成 5 项任务：选择紧急监测、选择日常监测、处理一次跌倒、处理一次激越、自由探索主页。任务清单直接对应三个场景加一个自由探索。", "【T 测试·混合到场 + 中途微调】6 人中 2 人用 Teams 远程、4 人现场；每场约 60 分钟并录音。前 4 场做完后先微调设计，最后 2 人评价的是改过的版本——用最后两个人当作迭代验证，一分钱不多花。", "【D 定义·分析动作】Amberscript 转录、Atlas.ti 分析；期望与前提用归纳主题分析，PSD 功能体验用归纳—演绎结合。两名研究者分别读荷兰语与英语文本，初始一致率 75%，讨论至完全共识。", "【不属于五步法】构思格缺席；测的是概念流程而不是可运行的传感系统，绝不能声称真实环境可用性或临床效果。"],
  ...
  "Draw-Care（澳洲 Monash）": {
    ...
    procedure: ["【E 同理 + P 原型·前序阶段】先由多元族裔照护者共同设计 Draw-Care，做完原型后回头找原共同设计参与者做 member checking（把成品拿回去问「这是不是你说的意思」）。这一步是共同设计合法性的关键动作，成本极低却极少有人做。", "【D 定义·先量能力】用户测试前先填 eHEALS，以及一份「互联网对你做健康决策和找资源有多重要/多有用」的调查。目的同前：把「产品难用」和「这个人本来就不上网」分开。", "【T 测试·30 人各 20–30 分钟】研究者邮件发登录码；每人在线屏幕共享 20–30 分钟，按固定顺序走：选语言 → 登录 → 进入 Films and More Information → 完成代表性任务，全程出声思考。多语言产品把「选语言」当第一个任务，是因为这一步最容易直接卡死。", "【T 测试·工具与取舍】用 Hotjar 自动记录网站行为（点击、路径、停留）；会谈刻意不录音，改由 2 名研究者现场详记问题——省了转录成本，代价是质性材料依赖笔记，作者自己在局限里承认了这一点。", "【P 原型·迭代】按测试结果改四样具体东西：虚拟助手、评分方式、反馈按钮、翻译。发现 14/30 人找不到或用不了虚拟助手，越南语和普通话有翻译偏差——多语言产品的偏差只能靠母语用户测出来，机器翻译审不出来。", "【D 定义·即时分析】即时把问题标为致命 / 严重 / 外观三档并观察是否饱和；SPSS 28 做描述统计；两名研究者独立做主题分析、协商编码，第 3 人复核调解。样本不足以按语言分组比较，作者明确没有硬比。"],
  ...
  "瑞典 tailor-made mobile app（Karolinska）★两篇成套": {
    ...
    procedure: ["【P 原型 + T 测试·合一】12 名居家失智症患者的家庭照护者在真实照护过程中使用一个定制 App 与专业人员沟通。没有实验室环节，App 本身在研究开始前已存在。", "【E 同理】对这 12 人做半结构化访谈，问的是「支持是怎么发生的」，不是「界面好不好用」。", "【D 定义·理论先行的分析】用演绎式质性内容分析（先有编码框架再套资料）来解释价值共创（value co-creation）：结论是 App 的价值产生于照护者与专业人员的互动之中，而不是软件单方面提供的。这个立场对我们有直接用处——任何「软件 + 真人」的干预都可以拿它当理论外衣。", "【E 同理·第二篇换视角】2024 年那篇把镜头转到专业人员一侧，分析提供支持与接受支持双方的体验。同一个项目、两种视角、两篇论文。", "【诚实标注】2022 篇实际发表于 BMC Health Services Research（不是 BMC Geriatrics）；2024 篇完整样本数因全文访问限制未能确认，此处不补写数字。"],
  ...
  "Olera.care（美国）": {
    ...
    procedure: ["【不属于五步法·招募工程】候选人通过扫二维码、打开网页或打电话三种通道完成资格筛查与电子知情同意。三通道并行是为了不把不擅长上网的照护者筛掉——而这恰恰是目标人群。", "【E 同理 + T 测试·第一轮】2022 年 1–5 月每人接受两轮 Zoom 或电话会谈。第一轮先了解本人的照护需求（同理），再引导他开始使用平台并在测试环境里探索主要功能（测试）。两件事压在一次会里，省一次约人。", "【T 测试·第二轮】第二轮完成 Qualtrics 调查与 13 项修订版 MARS（移动应用评分量表：参与度、功能性、美观、信息质量、主观质量，均 1–5 分）；研究者同时实时观察交互问题。用「修订版」而非完整 MARS 是明确的成本取舍，作者在局限里写明了。", "【D 定义·顺手采需求】同一份调查里顺便问了人口学、照护经历、技术沟通偏好，以及「你以前用过哪些老年服务、以后想了解哪些」。这是极高性价比的一招：评价问卷里夹一段市场需求调研，一次采集两种用途。", "【T 测试·结果与分析】描述统计汇总评分（总分 4.57/5），并用双样本双侧 t 检验比较不同照护者特征的 MARS 差异（照护时数多者功能评分更高，P=.02）。另有 21/30 人偏好「先匿名互动、再获个性化反馈」——这条偏好对我们做产品有直接价值。", "【不属于五步法】同理之外的前段全缺：平台早已存在，本篇是初步可用性评价，不能证明长期使用或照护效果；样本以白人、女性、经济稳定者为主。"],
  ...
  "Rathnayake mHealth 共同设计（澳洲 Griffith/JCU）": {
    ...
    procedure: ["【E 同理·三源并采】第一阶段用三种材料做需求评估：在线问卷（广度）、深度访谈（深度）、专家咨询（专业校正）。参与方包括家庭照护者、老年护理护士、医生、作业治疗师与 IT 专家，但摘要没有给出精确人数——这一点必须照实说明。", "【D 定义】把三路需求收敛成明确的 App 内容与功能清单。", "【I 构思 + P 原型】第二阶段与参与者共同设计界面并做出 Android 原型；构思与原型两格连着走，没有记录发散了几个方案。", "【P 原型·最终结构】定稿为三个模块：①失智与照护概述 ②日常生活活动（ADL）管理 ③照护者自身健康与福祉。这个三段式结构（知识 / 干活 / 照顾自己）几乎是所有照护者 App 的通用骨架，可以直接拿来当我们的信息架构起点。", "【不属于五步法·作者自认】测试格缺席：作者明确写明发布前仍需可行性与有效性测试，本文没有证明任何一项。"],
  ...
  "AreaAlzheimer（西班牙·加泰罗尼亚）★两篇成套": {
    ...
    procedure: ["【E 同理·大样本 + 分层小组】第一阶段向 210 名符合条件的照护者发 27 项结构化问卷（量的广度），另按两个维度分层组织 4 个焦点小组：参与者年龄、以及与患者的关系（配偶 / 子女等）。按关系分层是必须的——配偶照护者和子女照护者的需求几乎不重叠。", "【D 定义·需求四类 → 板块四块】把需求归成四类（信息指导、后勤协助、情绪与沟通策略、同伴社交连接），再一对一转成平台的四个板块：信息、培训、社区、研究。需求类别与产品板块一一对应，是最容易在论文里说清的映射方式。", "【I 构思·用评分做取舍】第二阶段另招 147 人评价不同数字资源的内容与有用性，据此决定优先做什么——最终优先保留疾病信息、任务管理、情绪支持三项。这一步是用投票替代头脑风暴，把「砍功能」这件难事外包给用户。", "【P 原型 + T 测试·线上线下双通道】开发 beta 版后，40 名照护者评价：21 人在线、19 人到基金会现场。任务是四件真实动作：浏览信息、参加论坛、注册支持项目、调整设置。", "【T 测试·现场组多测一层】只有现场组额外记录标准任务的难度、耗时与成功率，并现场讨论体验与改进机会。线上组测态度，现场组测行为——两类数据成本差很多，所以只在一部分人身上花贵的钱。", "【T 测试·结果】可及性 4.5/5，SUS 74.3/100（可接受），有用性只有 3.4/5。这个落差是全表最有信息量的一条：好用不等于有用；低数字素养者仍报告导航困难。全项目 419 是各阶段人次合计，不是单一试验样本，引用时不能含糊。"],
  ...
  "Mobile Care Ecosystem（新加坡 Duke-NUS）": {
    ...
    procedure: ["【E 同理·四类利益方】用目的抽样做需求深访，22 人涵盖四类：11 名家庭照护者、6 名社区失智服务人员、3 名临床人员、2 名政策制定者。把政策制定者放进需求访谈是刻意的——将来要靠他们推广。", "【D 定义】录音转录后把所需功能收敛为三类：教育、沟通、清单/目录。三类之外的许愿一律不进产品。", "【P 原型·理论 + 功能清单】依据压力过程模型（stress process model）与用户中心设计开发 CareBuddy，一次整合八件东西：个性化评估、AI 聊天、GPS、同伴支持、热线、远程医疗、服务连接、自我照护。这和我们的产品最像，但也正因为组件太多，最后无法判断到底是哪一项起作用（作者自己承认）。", "【T 测试·第 1 阶段 3 轮迭代】18 人参与、共 3 轮、每场 1–2 小时访谈：参与者用研究者的手机操作原型（不用自己的，省去安装问题），按话题指南逐项评论，每一轮结束后立刻更新 App 并让参与者填 SUS。SUS 从第 1 轮 65.4 升到第 3 轮 73.8，越过 68 的可接受基准——这条曲线本身就是「迭代有效」的最佳论文素材。", "【T 测试·第 2 阶段真机 1 个月】另 10 名照护者在自己的 Android/iOS 手机上用满 1 个月；后台记录每周参与时长、每个组件各用了多久、哪些功能最常用。换到自己手机是关键一跳，因为安装、通知、耗电这些问题只有真机会暴露。", "【T 测试·月末收口】月末填 18 项 MAUQ（易用性、界面/满意度、有用性，总分 95.4），再做 10–25 分钟开放式访谈。用户重视失智管理、自我照护、社交网络、服务目录与对话模型，但明确要求减少文字并改善导航——「字太多」是所有照护者 App 的通病，值得我们直接引用。"],
  ...
  "家庭用药管理可用性协议（Quintana 等）": {
    ...
    procedure: ["【P 原型 + T 测试·设备与账号全由研究方给】在办公室提供装有公开版 InfoSAGE 的开发用 iPad 和测试账号，开场只简述平台、分级权限（家庭成员能看到多少）与主要功能，刻意不教具体操作——因为要测的正是「没人教能不能会」。", "【T 测试·8 个高风险场景】参与者边出声思考边完成 8 个场景，全部围绕用药：添加 3 种药、查副作用、查药物相互作用、修改华法林剂量、停用一种药、把药物清单邮件发出去。选华法林是因为它剂量错就出事——测试场景刻意挑最危险的动作，这一点非常值得抄。", "【T 测试·三路录制 + 隐私处理】同时录音、录屏，视频只拍手部（不拍脸，规避隐私顾虑因而更容易过伦审并降低参与者抵触）。仅在参与者完全无法继续时才给帮助并记录一次；每个场景按其全部子事件是否完成来判定成功，不按主观感觉。", "【T 测试·后测】完成后填 1–7 分修订版可用性问卷（满意度、难度、潜在使用意愿共 5 项），再做开放式访谈。", "【D 定义·分析计划】计划转录并做主题编码；2 名分析者用 BORIS 7 加共享代码本独立编码视频与屏幕，团队裁决差异并计算双向组内相关；另用时间序列聚合问题，并比较前两个结构相同的场景所用时间以估计学习曲线。「用两个同构任务的耗时差算学习曲线」是很便宜又很显专业的一招。", "【不属于五步法·重要事实】这是协议（protocol），只计划 20 人（10 名照护者 + 10 名衰弱高龄老人）、受控办公室、指定 iPad；无任何实证结果，也不是失智症专属。绝不能引用为「该 App 已被证明可用」。"],
  ...
  "配偶照护者身体活动 App 设计（Penn State，确认含患者本人）": {
    ...
    procedure: ["【E 同理·招募单位是「一对」】以配偶二元组（dyad）为单位招募 14 对、共 28 人，且确实包含失智症患者本人，不是只问家属代答。二元组招募比单人贵得多、慢得多，但它是唯一能拿到「两人之间怎么协商」的方式。", "【E 同理】围绕五个话题做半结构化访谈：运动偏好、现有习惯、障碍、动机、以及希望 App 提供什么支持。问的是生活，不是软件功能。", "【D 定义】访谈逐字转录，保证后续编码有据可依。", "【D 定义】做主题分析，得出 4 大主题：运动偏好与习惯、运动障碍、运动动机、App 偏好与支持功能，再把主题逐条转成设计方向。", "【不属于五步法】构思、原型、测试三格全部缺席：这是纯需求研究，连原型都没有，因此不能推断任何运动或健康效果。它的正确用法是当「我们的功能有需求依据」的引文。"],
