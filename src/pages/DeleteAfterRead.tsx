...
const papers: Paper[] = [
  ...
  },
  {
    ...
    procedure: ["【E 同理·仅计划】Aim 1（NIH Stage 0）：写明将来要评估家庭照护者在用药管理上的需求——谁在管药、怎么记、在哪出错。注意是「计划要做」，本文没有任何访谈数据。", "【I 构思 + P 原型·仅计划】Aim 2（Stage IA）：写明将来要和照护者一起共同设计一个信息技术干预原型，两格被压在同一句里，没有说明发散几轮、用什么材料。", "【T 测试·仅计划】Aim 3（Stage IB）：写明将来要做原型的可行性测试。", "【不属于五步法】本文是协议（protocol），在执行之前先把方法登记发表，因此没有结果、没有样本量完成值、没有任何可引用的效果。它的全部价值就是「先占位、后交付」这一个动作本身。"],
  ...
  },
  {
    ...
    procedure: ["【P 原型】把《正向失智症照护 30 天指南》（PDC30）整本转成 GPT-4o 机器人的受控知识库——即知识边界被限死在这一本书里，不让模型自由发挥。这就是全篇唯一的「做东西」动作。", "【D 定义·靠对照组划线】不做用户需求调研，改用两个对照来定义「什么算好回答」：对照一是公开版 ChatGPT（无知识库）；对照二是 Chatbot-B，载入 WHO iSupport、机构指南等共 185 份资料（知识库大但杂）。", "【T 测试·机器对机器】研究者用同一组 21 个常见照护问题分别问三个机器人，逐题横向比较答案。这一步没有真人参与，属于纯模型评测。", "【T 测试·真人】21 名家庭照护者实际使用 PDC30 机器人 2 周，随后给评分并提交书面评论（哪里有用、哪里答歪、哪里不敢信）。", "【T 测试·收口】研究者把定量评分和书面反馈的主题合并解读；没有随机分组，没有健康结局，因此只能说「可接受」，不能说「有效」。", "【不属于五步法】同理与构思两格完全缺席：内容来源是一本现成的书，没有访谈，也没有考虑过别的形态（例如不做机器人而做清单）。"],
  ...
  },
  {
    ...
    procedure: ["【D 定义】研究者自己汇编 32 个代表性的早期失智症照护问题作为评测集。定义这一格由专家完成，没有照护者参与选题。", "【P 原型·只改提示词】不写代码、不训练模型：C1 = 公开基线 ChatGPT-4o 原样；C2 = 同一模型加上提示工程（prompt engineering）后的增强版。「原型」的全部内容就是一段提示词。", "【T 测试】两个版本各回答同样 32 题，共产出 64 个答案，成对可比。", "【T 测试】12 名专家用已验证过的评分指标对 64 个答案逐一打分，评分者不是照护者本人而是领域专家。", "【T 测试 + E 同理·倒序】先比较 C1 与 C2 的评分差异，再回头访谈同一批专家，对访谈做主题分析，解释「为什么增强版更好/仍不够」。同理格出现在最后，是给评分结果找解释，不是给设计找方向。", "【不属于五步法】构思格缺席，也没有做第三版提示词——比较完就结束。"],
  ...
  },
  {
    ...
    procedure: ["【P 原型·别人的循证内容】被测对象是 GamePlan4Care：把已获证据支持的 REACH II 照护者干预搬上网页。本篇不负责发明内容，只负责在正式随机试验之前把界面和内容挑毛病。", "【D 定义·分工测两层】31 名照护者被按测试目的拆成两组：18 人专测技术（能不能点得动、导航是否走得通），13 人专测内容（说的东西对不对、看不看得懂）。用分组代替需求定义。", "【T 测试】参与者在系统里执行指定任务，边操作边回答开放式问题，随时反馈使用感受；研究者不指路。", "【T 测试·留证】全程同步录制音频与屏幕，事后逐字转录，保证卡点可回看、可计数。", "【D 定义·反哺开发】两名研究者依次熟悉资料 → 归纳编码 → 形成主题，最后把导航问题和内容问题整理成可执行的修改单交回开发团队。修改后的版本不在本篇，本篇只交问题。", "【不属于五步法】同理与构思缺席（内容源自 REACH II 既有干预），也不测临床疗效——形成性研究的定位就是「先别翻车，再去做 RCT」。"],
  ...
  },
  {
    ...
    procedure: ["【D 定义·先量人不量产品】参与者进场先填两份：人口学资料 + 意大利语版 eHEALS（电子健康素养量表）。这一步是给后面的可用性分数找解释变量——分低到底是产品烂还是这个人本来就不会用电脑。", "【P 原型·借来的】被测对象是 WHO iSupport 的瑞士意大利语文化适配桌面版，翻译与适配在本篇之前已完成，本篇不做开发。", "【T 测试】12 人（10 名非正式照护者 + 2 名正式照护者）逐一完成 11 项结构化任务，全程出声思维，研究者在旁观察并记录完成与卡顿过程。", "【T 测试】任务结束立刻填 SUS（系统可用性量表）与信息评价题（信息质量、信息数量、导航等）。这批分数就是本篇的定量骨架。", "【E 同理·收尾】最后做半结构化访谈，把量表打不出来的东西问出来，归纳成内容、可信度、导航、互动性、情感影响等主题。", "【不属于五步法】没有原型迭代、没有 v2、没有构思：全文是一次性验收，任务清单也由研究者单方面拟定。"],
  ...
  },
  {
    ...
    procedure: ["【T 测试·仅计划·第一轮】设计一场走查工作坊，由早期失智症居家生活者本人（不是家属代答）评价 Hear Me Now App，目标是沟通、安全与福祉三方面。", "【T 测试·仅计划·第二轮】计划让最多 12 个家庭把 App 带回家真实使用，取代实验室情境。", "【P 原型·仅计划】根据前两轮资料修订 App，形成修订版。", "【T 测试·仅计划·第三轮】再办一场走查工作坊评估修订版，构成「测 → 改 → 再测」闭环。", "【D 定义·仅计划】计划整合不少于 4 种资料来源，并用常态化过程理论（Normalization Process Theory）解释「为什么会被采用/为什么用不起来」。", "【不属于五步法·重要事实】这是探索性多案例研究方案；因 COVID，本文没有可引用的执行数据与结果，全部步骤都停留在「计划」层面。引用它只能引方案设计，绝不能当成患者共创的成果。"],
  ...
  },
  {
    ...
    procedure: ["【D 定义·文献层】先写检索策略与纳入排除标准：只要真的被研究过的失智照护 App，把纯商业宣传和没有实证的产品排掉。", "【D 定义】两阶段筛选——先筛标题与摘要，再读全文定稿，形成最终纳入集合。", "【D 定义】逐篇提取三类特征：App 有哪些功能、安全性（数据与用药安全等）如何被处理、可用性怎么测的。", "【D 定义·汇总】跨研究合并，指出共同缺口与未来研究方向。", "【不属于五步法】这是系统综述，全篇只落在「定义」这一格：没有同理（不接触任何照护者）、没有构思、没有原型、没有测试。它的正确用法是当弹药库——引它来证明「现有 App 缺 X」，不是引它来证明任何设计做法有效。"],
  ...
  },
  {
    ...
    procedure: ["【P 原型·唯一可抄的动作】把同一个健康功能做成 3 个不同界面版本（同功能、不同呈现）。这是本篇唯一值得抄的设计动作：变量只留界面一个，别的全固定。", "【T 测试】老年人与照护者两类用户分别体验被指定的版本，形成界面 × 用户类型的交叉。", "【T 测试】各自完成可接受性评价（愿不愿意用、看不看得懂、界面观感）。", "【T 测试·比较】比较两件事：不同界面版本之间的评分差异，以及同一界面上老年人与照护者的评分差异。后者往往才是有话可说的地方——同一个界面，被照护者和照护者接受度并不一致。", "【不属于五步法】同理、定义、构思三格缺席：功能是给定的，界面版本由研究者自定，没有用户参与发散。"],
  ...
  },
  {
    ...
    procedure: ["【P 原型·已上线产品】被研究对象是已经做好的 CareVirtue 网络平台，本篇不参与开发。第一步是主要照护者注册并把自己的支持者（子女、亲属、朋友、邻居）拉进来组成照护网络——注意这一步本身就是最大的失败点，网络能不能建起来决定后面一切。", "【T 测试·真实使用】网络成员在日常照护里使用三类功能：共享日志（谁做了什么、老人今天怎么样）、任务（分派与认领）、沟通。使用时长按可行性研究设定，不是实验室 30 分钟。", "【T 测试·定量】研究团队采集后台使用数据（登录、发帖、任务完成等）加问卷，用来判断可行性与可接受性，不判断疗效。", "【E 同理·解释用】访谈补充「数字看不出来的部分」：为什么有人不发帖、家庭内部谁不愿意被看见、共享日志会不会变成互相监督。", "【不属于五步法】前三格（同理、定义、构思）在本篇缺席——它是产品先存在、研究后跟进的典型；这也是为什么它比多数共同设计论文有参考价值。"],
  ...
  },
  {
    ...
    procedure: ["【P 原型】给每位照护者配置一套定制 App，并配上真人专业支持——即干预是「软件 + 人」的组合，不是纯自助工具。评估时这两者无法拆开，是本篇的结构性局限。", "【T 测试·试点期】试点期内，照护者通过 App 接收针对本人情况的信息推送与沟通支持（不是统一群发内容）。", "【T 测试·前后测】干预前后各填一次压力与抑郁相关量表，构成前后对照。没有随机对照组，所以变化不能归因于干预本身。", "【E 同理·事后】访谈参与者：哪一类支持真的用上了、哪一类打开一次就不再看、还缺什么。这一格出现在最后，是解释而非驱动设计。", "【不属于五步法】定义与构思缺席：定制规则来自专业人员判断，没有记录任何发散或备选方案设计过程。"],
