...
const papers: Paper[] = [
  {
    ...
    procedure: ["向参与者介绍可用语音或文字输入、以音频/文字/视觉输出的 Lola。", "依次完成 3 项任务：查找脑健康文章、查找本地活动、查找参加研究的机会。", "参与者边操作边表达想法；研究者记录操作、意见和总体印象。", "研究团队按可用性、可及性、文化相关性和采用度整理反馈。"],
  ...
  },
  {
    ...
    procedure: ["第一段只看首页并说出第一印象，不执行任务。", "第二段按研究团队规定的情境完成网站任务，同时出声思维。", "第三段进行半结构化访谈，补充任务中没有说出的感受。", "录音转录后，同一份资料分别套入 ISO、Nielsen 与 Garrett 三套框架。"],
  ...
  },
  {
    ...
    procedure: ["分别访谈照护者和工作人员，收集对原版内容、措辞、视频和交付方式的体验。", "研究团队迭代整理资料，制作供工作坊讨论的触发材料。", "13 名照护者和 17 名工作人员参加共同设计工作坊，提出并排序适配建议。", "Adaptation Working Party 对建议进行最终裁定并落实为英国版修改。"],
  ...
  },
  {
    ...
    procedure: ["依据 mHealth 开发建议和 SATURN 测试制作 V1。", "焦点小组操作 V1，采用出声思维并填写 SUS；团队汇总最严重问题。", "修改语言、文化表达、导航与题目措辞，制作 V2。", "新一轮焦点小组重复测试；末轮因参与者难以理解 SUS，未强行生成不可靠分数。"],
...
const projectDossiers: Record<string, ProjectDossier> = {
  ...
  "家庭用药管理可用性协议（Quintana 等）": {
    ...
    participants: "协议计划面向高龄老人及参与其药物管理的家属；本文不含已完成招募或结果。",
    procedure: ["界定家庭药物管理任务与目标用户。", "规定可用性测试场景、任务和观察流程。", "预先选择测量与分析方法。", "把整套方法作为 protocol 发表，等待后续执行。"],
    measures: "协议预设的任务表现和可用性资料；本页无法从受限 PDF 确认具体量表名称，因此不猜。",
    analysis: "只描述未来分析方法，没有实际数据分析。",
    results: "无实证结果；贡献是协议本身。",
    limits: "不是失智症专属，也不是结果论文；不能引用为 App 已可用或有效。"
  },
