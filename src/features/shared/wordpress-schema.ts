...
export const wordpressSchema: Record<WordPressFeatureKey, WordPressSchemaEntry<any, any, any>> = {
  ...
  activity_logs: {
    status: "confirmed",
    endpoint: "wp/v2/activity_log",
    defaultParams: { per_page: 50 },
    mapList: (items: WPPostEntity[]) => (Array.isArray(items) ? items.map(mapAcfPost) : []),
    buildCreateBody: (input: { user_id: string; action: string; activity_type?: string; entity_type?: string; entity_id?: string }) => ({
      title: input.action,
      status: "publish",
      acf: { user_id: input.user_id, action: input.action, activity_type: input.activity_type || "general", entity_type: input.entity_type, entity_id: input.entity_id },
    }),
  },
  saved_providers: {
    status: "provisional",
    endpoint: "cc/v1/favorites",
    defaultParams: { per_page: 100 },
    mapList: (items: any[]) => (Array.isArray(items) ? items.map((s) => ({
      id: String(s.id ?? s.favorite_id ?? s.provider_id ?? s.store_id),
      provider_id: s.provider_id ? `wp-${s.provider_id}` : (s.store_id ? `wp-${s.store_id}` : null),
      provider_name: s.provider_name || s.store_name || s.name || null,
      provider_avatar: s.provider_avatar || s.avatar || s.store_avatar || null,
    })) : []),
    buildCreateBody: (input: { provider_id: string }) => ({ provider_id: parseInt(input.provider_id.replace("wp-", ""), 10) }),
  },
  saved_provider: {
    status: "provisional",
    endpoint: ({ id }) => `cc/v1/favorites/${id}`,
  },
  location_shares: {
    status: "provisional",
  ...
  symptom_logs: {
    status: "confirmed",
    endpoint: "wp/v2/symptom_log",
    defaultParams: { per_page: 100 },
    mapList: (items: WPPostEntity[]) => (Array.isArray(items) ? items.map(mapAcfPost) : []),
    buildCreateBody: (input: { symptom_type: string; severity?: number }) => ({
      title: input.symptom_type,
      status: "publish",
      acf: { symptom_type: input.symptom_type, severity: input.severity },
    }),
  },
  wellness_logs: {
    status: "confirmed",
    endpoint: "wp/v2/wellness_log",
    defaultParams: { per_page: 100 },
    mapList: (items: WPPostEntity[]) => (Array.isArray(items) ? items.map(mapAcfPost) : []),
    buildCreateBody: (input: { user_id: string; mood?: string; stress_level?: number; notes?: string }) => ({
      title: "Wellness Log",
      status: "publish",
      acf: { user_id: input.user_id, mood: input.mood, stress_level: input.stress_level, notes: input.notes },
    }),
  },
  emergency_contacts: {
    status: "confirmed",
