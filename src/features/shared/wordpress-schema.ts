...
export const wordpressSchema: Record<WordPressFeatureKey, WordPressSchemaEntry<any, any, any>> = {
  ...
  care_tasks: {
    status: "confirmed",
    endpoint: "jet-cct/care_task_real",
    defaultParams: { _limit: 100 },
  ...
  care_task: {
    status: "confirmed",
    endpoint: ({ id }) => `jet-cct/care_task_real/${id}`,
    mapDetail: (t: any) => ({
      id: String(t._ID || t.id),
      title: t.a || "Task",
      status: String(t.l ?? "1") === "2" ? "completed" : "pending",
  ...
  care_plan: {
    status: "confirmed",
    endpoint: ({ id }: { id: string }) => `jet-cct/care_plan/${id}`,
    mapDetail: (p: any) => p,
  },
  care_plan_goals: {
    status: "confirmed",
    endpoint: "jet-cct/care_plan_goal",
    defaultParams: { _limit: 100 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
  ...
  location_share_me: {
    ...
    mapDetail: (items: any[]) => {
      ...
      return {
        ...
        is_sharing_enabled: l.enabled === true || l.enabled === "yes",
        updated_at: l.updated_at || l.created_at,
      };
    },
    buildUpdateBody: (input: { latitude: number; longitude: number; is_sharing_enabled?: boolean }) => ({
      latitude: input.latitude, longitude: input.longitude, enabled: input.is_sharing_enabled ? "yes" : "no",
    }),
  },
  conversations: {
    status: "confirmed",
    endpoint: "jet-cct/chat_conversation",
    defaultParams: { _limit: 50 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
  },
  conversation_messages: {
    status: "confirmed",
    endpoint: "jet-cct/chat_message",
    defaultParams: { _limit: 100 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
  },
  conversation_message: {
    ...
    buildCreateBody: (input: { conversation_id: string; content: string }) => ({
      conversation_id: input.conversation_id, content: input.content,
    }),
  },
  chat_participants: {
    status: "confirmed",
    endpoint: "jet-cct/chat_member",
    defaultParams: { _limit: 100 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { conversation_id: string; user_id: string }) => ({
      conversation_id: input.conversation_id, user_id: input.user_id,
    }),
  },
  // ─── Health Domain ─────────────────────────────────────────
  checkins: {
    ...
    buildCreateBody: (input: { mood?: string; sleep_hours?: string; notes?: string }) => ({
      activity_type: "checkin", mood: input.mood, sleep_hours: input.sleep_hours, notes: input.notes,
    }),
  },
  medicines: {
    status: "confirmed",
    endpoint: "jet-cct/medicine",
    defaultParams: { _limit: 100 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { name: string; dosage?: string; frequency?: string }) => ({
      name: input.name, dosage: input.dosage, frequency: input.frequency,
    }),
  },
  medicine_logs: {
    status: "confirmed",
    endpoint: "jet-cct/medicine_log",
    defaultParams: { _limit: 100 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { medicine_id: string; administered_by?: string; status?: string }) => ({
      medicine_id: input.medicine_id, administered_by: input.administered_by, status: input.status || "taken",
    }),
  },
  health_vitals: {
    status: "confirmed",
    ...
    buildCreateBody: (input: { vital_type: string; value: string; unit?: string }) => ({
      vital_type: input.vital_type, value: input.value, unit: input.unit,
    }),
  },
  symptom_logs: {
    status: "confirmed",
  ...
  emergency_contacts: {
    status: "confirmed",
    endpoint: "jet-cct/emergency_contact",
    defaultParams: { _limit: 50 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { name: string; phone: string; relationship?: string }) => ({
      name: input.name, phone: input.phone, relationship: input.relationship,
    }),
  },
  job_postings: {
    status: "confirmed",
    endpoint: "jet-cct/job_posting",
    defaultParams: { _limit: 50 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (job: { title: string; description: string; location?: string; start_date?: string }) => ({
      title: job.title, description: job.description, location: job.location, start_date: job.start_date, status: "open", job_source_type: "general",
    }),
  },
  job_applications: {
    status: "confirmed",
    endpoint: "jet-cct/job_application",
    defaultParams: { _limit: 50 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
  ...
  job_application: {
    ...
    buildUpdateBody: (input: { status: string }) => ({ status: input.status }),
  },
  // ─── Location / Safety ─────────────────────────────────────
  location_history: {
    status: "confirmed",
  ...
  location_requests: {
    status: "confirmed",
    endpoint: "jet-cct/location_request",
    defaultParams: { _limit: 50 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { requester_id: string; target_user_id: string; message?: string }) => ({
      requester_id: input.requester_id, target_user_id: input.target_user_id, status: "pending", message: input.message,
    }),
  },
  safe_zones: {
    status: "confirmed",
    endpoint: "jet-cct/safe_zone",
    defaultParams: { _limit: 50 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { name: string; latitude: number; longitude: number; radius_meters: number }) => ({
      name: input.name, latitude: String(input.latitude), longitude: String(input.longitude), radius_meters: String(input.radius_meters), is_active: true,
    }),
  },
  safe_zone_alerts: {
    status: "confirmed",
    endpoint: "jet-cct/safe_zone_alert",
    defaultParams: { _limit: 50 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { safe_zone_id: string; alert_type: string; latitude?: number; longitude?: number; message?: string }) => ({
      safe_zone_id: input.safe_zone_id, alert_type: input.alert_type, latitude: input.latitude ? String(input.latitude) : undefined, longitude: input.longitude ? String(input.longitude) : undefined, message: input.message, is_read: false,
    }),
  },
  // ─── Facility Membership / Ownership ───────────────────────
  entity_members: {
    status: "confirmed",
    endpoint: "jet-cct/entity_member",
    defaultParams: { _limit: 100 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { entity_id: string; user_id: string; role?: string; is_owner?: boolean }) => ({
      entity_id: input.entity_id, user_id: input.user_id, role: input.role || "member", is_owner: input.is_owner || false, is_admin: false,
    }),
  },
  ownership_claims: {
    status: "confirmed",
    endpoint: "jet-cct/ownership_claim",
    defaultParams: { _limit: 50 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { entity_id: string; user_id: string; claim: string }) => ({
      entity_id: input.entity_id, user_id: input.user_id, claim: input.claim, status: "pending",
    }),
  },
  ownership_disputes: {
    status: "confirmed",
    endpoint: "jet-cct/ownership_dispute",
    defaultParams: { _limit: 50 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { entity_id: string; user_id: string; claim: string }) => ({
      entity_id: input.entity_id, user_id: input.user_id, claim: input.claim, status: "pending",
  ...
  votes: {
    status: "confirmed",
    endpoint: "jet-cct/vote",
    defaultParams: { _limit: 100 },
    mapList: (items: any[]) => (Array.isArray(items) ? items : []),
    buildCreateBody: (input: { entity_id: string; entity_type: string; user_id: string; value: number }) => ({
      entity_id: input.entity_id, entity_type: input.entity_type, user_id: input.user_id, value: input.value,
    }),
  },
  generic_posts: {
    status: "provisional",
