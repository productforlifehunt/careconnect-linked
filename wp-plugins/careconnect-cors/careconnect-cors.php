...
// ─── 5. Quote → hidden WooCommerce product (server-side, elevated) ───
//    Buyers don't have product-create caps, so the WC REST API rejects
//    them with 403. This endpoint accepts an authenticated quote payload
//    from any logged-in user and creates the hidden simple product on
//    behalf of the seller (so Dokan attributes commission correctly).
add_action('rest_api_init', function () {
    register_rest_route('careconnect/v1', '/quote-product', array(
        'methods'             => 'POST',
        'permission_callback' => function () { return is_user_logged_in(); },
        'callback'            => 'cc_create_quote_product',
    ));
});

function cc_create_quote_product(WP_REST_Request $req) {
    ...
    // Attribute to vendor so Dokan picks up commission split.
    ...
    // Set the post_author = vendor so Dokan treats it as a vendor product.
