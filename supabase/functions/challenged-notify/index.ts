/**
 * ChallengeD — the ONE notification endpoint for this app.
 * Every ChallengeD in-app / push / email / SMS message is dispatched here.
 */
...
Deno.serve(async (req) => {
  ...
  try {
    const results = [];
    for (const item of items) results.push(await dispatch("challenged", item, wpToken));
    return new Response(JSON.stringify({ app: "challenged", results }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
