...
<h1 class="wp-block-heading" style="margin-top:0;margin-bottom:0.25rem;font-size:1.5rem;font-weight:700">Appointments</h1>
<!-- /wp:heading -->
<!-- wp:paragraph {"style":{"color":{"text":"#6b7280"},"typography":{"fontSize":"0.875rem"}}} -->
<p class="has-text-color" style="color:#6b7280;font-size:0.875rem">Manage your care bookings and appointments.</p>
<!-- /wp:paragraph -->
<!-- wp:spacer {"height":"1rem"} -->
<div style="height:1rem" aria-hidden="true" class="wp-block-spacer"></div>
<!-- /wp:spacer -->"""
BLOCKS = ["bookings-tabs-block.html", "bookings-list-block.html"]
parts = [HEADER]
for name in BLOCKS:
    with open(os.path.join(BASE, name), 'r') as f:
        parts.append(f.read().strip())
content = COLUMNS_OPEN + SIDEBAR + "\n" + MAIN_COLUMN_OPEN + "\n" + os.linesep.join(parts) + "\n" + MAIN_COLUMN_CLOSE
payload = {"id": 162, "content": content, "status": "publish"}
out = os.path.join(BASE, "bookings70_payload.json")
